...
export function CartItemDisplay({
  ...
  if (item.isCombo && item.comboItems) {
    ...
    return (
      <Card className={`border-green-300 shadow-sm transition-all bg-gradient-to-br from-green-50/30 to-emerald-50/30`}>
          ...
          {item.comboExpanded && item.comboItems && (
            <div className="ml-8 space-y-2 mb-3 pb-3 border-t border-green-200 pt-3">
              {item.comboItems.map((subItem, idx) => {
                ...
                return (
                  <div key={idx} className="bg-white/80 rounded-lg p-2.5 border border-green-200">
                        ...
                        {subItem.customization && (
                          <div className="bg-slate-50 rounded-md p-1.5 mb-1.5 space-y-1">
                            <div className="flex items-center gap-2 text-xs text-slate-600">
                              <span>Đường: <span className="text-slate-800">{subItem.customization.sugarLevel}</span></span>
                              <span className="text-slate-300">•</span>
                              <span>Đá: <span className="text-slate-800">{subItem.customization.iceLevel}</span></span>
                            </div>
                            
                            {subItem.customization.toppings.length > 0 && (
                              <div className="flex gap-1 flex-wrap">
                                {subItem.customization.toppings.map((topping, tIdx) => (
                                  <Badge key={tIdx} variant="outline" className="text-xs px-1.5 py-0.5 bg-white border-blue-200 text-blue-700">
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.customization && (
              <div className="mb-2 bg-slate-50 rounded-md p-2 space-y-1">
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  <span>Đường: <span className="text-slate-800">{item.customization.sugarLevel}</span></span>
                  <span className="text-slate-300">•</span>
                  <span>Đá: <span className="text-slate-800">{item.customization.iceLevel}</span></span>
                </div>
                
                {item.customization.toppings.length > 0 && (
                  <div className="flex gap-1 flex-wrap">
                    {item.customization.toppings.map((topping, idx) => (
                      <Badge key={idx} variant="outline" className="text-xs px-1.5 py-0.5 bg-white border-blue-200 text-blue-700">
                        {topping.name}
                      </Badge>
                    ))}
                  </div>
            ...
            {!item.customization && item.toppings && item.toppings.length > 0 && (
              <div className="mb-2">
            ...
            {appliedPromoCode === 'GIAMCF' && item.name.toLowerCase().includes('cà phê') && (
