...
interface ComboSelectionPopupProps {
  ...
  onClose: () => void;
  combo: Combo | null;
  onConfirm: (selectedItems: { [groupId: string]: string[] }, combo: Combo) => void;
}
...
export function ComboSelectionPopup({
  ...
  const [selectedItems, setSelectedItems] = useState<{ [groupId: string]: string[] }>({});
  ...
  const handleItemSelect = (groupId: string, itemId: string, group: ComboGroup) => {
  ...
  const validateSelections = (): { valid: boolean; missingGroups: string[] } => {
                ...
                <div className="space-y-3">
                  ...
                  {group.maxSelect === 1 ? (
                    <RadioGroup 
                      ...
                      onValueChange={(value) => handleItemSelect(group.id, value, group)}
                    >
                    ...
                    <div className="space-y-2">
                      {group.items.map(item => {
                        const isSelected = selectedItems[group.id]?.includes(item.id);
                        return (
                          <div
                            key={item.id}
                            onClick={() => item.stock > 0 && handleItemSelect(group.id, item.id, group)}
